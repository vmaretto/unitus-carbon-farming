...
(function () {
  ...
  class StudyCompanionWidget extends HTMLElement {
    ...
    async _openSetupModal() {
      ...
      modal.innerHTML = `
        <div class="modal">
          <h2 style="font-size:18px; margin: 0 0 16px;">Definisci il tuo obiettivo</h2>
          <div class="field">
            <label>Obiettivo (testo libero)</label>
            <textarea name="goal" rows="2" placeholder="Es. superare l'esame finale del Master con voto alto">${escapeHtml(existing.goal || '')}</textarea>
          </div>
          <div class="field">
            <label>Data obiettivo</label>
            <input type="date" name="target_date" min="${todayIso}" value="${defaultDate}">
          </div>
          <div class="field">
            <label>Minuti di studio al giorno: <span id="mins-out">${existing.daily_minutes || 45}</span></label>
            <input type="range" name="daily_minutes" min="15" max="180" step="15" value="${existing.daily_minutes || 45}">
          </div>
          <div class="field">
            <label>Giorni della settimana disponibili</label>
            <div class="days">
              ${['L','M','M','G','V','S','D'].map((d, i) => `
                <label><input type="checkbox" name="day" value="${i+1}" ${(!existing.weekly_days || existing.weekly_days.includes(i+1)) ? 'checked' : ''}><span>${d}</span></label>
              `).join('')}
            </div>
          </div>
          <div class="field">
          ...
          <div class="field">
            <label>Moduli focus (opzionale)</label>
            <div class="modules-list" id="mods-list">
              ${modules.length === 0
                ? '<p class="muted" style="padding:8px;">Nessun modulo disponibile (verifica l\'iscrizione).</p>'
                : modules.map(m => `
                  <label>
          ...
          <div style="display:flex; gap:8px; justify-content:flex-end; margin-top: 16px;">
            ...
            <button class="btn btn-primary" id="save">${existing.id ? 'Aggiorna piano' : 'Genera piano'}</button>
          </div>
        </div>
      `;

      // Sopra a tutto, usiamo body perché siamo in Shadow DOM e position:fixed
      // su un elemento dentro lo Shadow non copre la viewport.
      ...
      $('#save').onclick = async () => {
        ...
        $('#save').disabled = true;
        $('#save').innerHTML = '<span class="spinner"></span> Genero...';
        try {
